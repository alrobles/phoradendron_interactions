library(tidyverse)
max_modularity_df <-   read_csv("data/output/tbl/modularity_mean_df.csv")
parasite_coefficients_mean <- read_csv("data/output/tbl/parasite_coefficients_mean.csv")
...
model_connectivity <- lm(data = train, formula = connectivity ~ s + mu  )
...
png("data/output/plots/s_mu_connectivity.png", 480*2, 480*2)
p1
dev.off()

pdf("data/output/plots/s_mu_connectivity.pdf", 11, 11)
p1
dev.off()
