...
biadjacency <- biadjacency[, selectedNames] %>%
  as.matrix(  )

rownames(biadjacency) <- rNames
phydissim <- log10(phydist + 1)/max(log10(phydist + 1))

parasitefilt <- which(rowSums(incidence) > 0)
biadjacency_filter <- biadjacency[parasitefilt, ]

modelList <- boostrap_lm_models(incidence_filter, phydissim, geodist, envdist, 5)
...
p1_boxplot <- output_pR2 %>%
  ...
  ylab("Model Name") +
  xlab("McFadden pseudo r-squared")
# 
...
parasitefilt <- which(rowSums(biadjacency) > 1)
biadjacency_filter <- biadjacency[parasitefilt, ]
...
parasite_coefficients_phy <- bind_rows(parasite_coefficients_phy_mean, parasite_coefficients_phy_all)
write_csv(parasite_coefficients_phy, "data/16_parasite_coefficients_phy.csv")
